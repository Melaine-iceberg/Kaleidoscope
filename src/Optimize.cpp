...
///
/// InitializeModuleAndManagers - Initializes the LLVM module and various
/// analysis managers. This function sets up the LLVM context, module, IR
/// builder, and various pass managers. It also registers the necessary analysis
/// passes and adds transform passes to the function pass manager.
///
...
void HandleDefinition() {
  if (auto fn_result = ParseDefinition()) {
    const auto fn_ast =
        std::move(*fn_result); // 提取 std::unique_ptr<FunctionAST>
    // 将函数原型添加到函数原型映射中
    if (const auto *fn_ir = fn_ast->codegen()) {
      std::fputs("Read function definition:", stderr);
    ...
    // 处理错误
    std::fprintf(stderr, "Error parsing definition: %.*s\n",
                 static_cast<int>(fn_result.error().size()),
                 fn_result.error().data());
    get_next_token();
  }
}

/// HandleExtern - Handle external declarations.
/// This function parses an extern declaration and adds it to the function
/// prototypes. It expects the extern to be in the form of`extern <name>(<args>)
void HandleExtern() {
  if (auto ProtoResult = ParseExtern()) {
    auto proto_ast =
        std::move(*ProtoResult); // 提取 std::unique_ptr<PrototypeAST>
    if (const auto *fn_ir = proto_ast->codegen()) {
      std::fputs("Read extern: ", stderr);
      fn_ir->print(llvm::errs());
      std::fputc('\n', stderr);
    ...
    // 处理错误
    std::fprintf(stderr, "Error parsing extern: %.*s\n",
                 static_cast<int>(ProtoResult.error().size()),
                 ProtoResult.error().data());
    get_next_token();
  }
}
...
/// @brief Retrieves a function by name from the current module or function
/// prototypes.
/// @param name The name of the function to retrieve.
/// @return  A pointer to the LLVM Function if it exists, or nullptr if it does
/// not.
